...
export default function RankingScreen({ currentPlayer, onBack, playerRankings, onViewDailyResults, selectedDate }: RankingScreenProps) {
  ...
  const canClaim = useCallback((rank: number, rowPlayer: string) => {
    ...
    // ✅ CORREÇÃO: Verificar se o jogador é winner verificando no ranking
    // O jogador é winner se está no top 3 e é o player atual
    const isWinner = rowPlayerLower === currentPlayerLower && rank <= 3
    
    // ✅ CORREÇÃO: Finalização determinada EXCLUSIVAMENTE por cálculo UTC local
    // Dias passados são SEMPRE considerados finalizados
    // Não usar estados, backend ou contrato para determinar finalização
    
    // 🔍 DIAGNÓSTICO: Verificar cada condição separadamente
    const checks = {
      isPastDay, // ✅ CORREÇÃO: Usa apenas cálculo UTC local
      isWinner,
    ...
    const canClaimResult = (
      checks.isPastDay && // ✅ Dia deve ser passado (finalizado)
      checks.isWinner && // ✅ Jogador deve ser winner
  ...
  const handleClaim = useCallback(async (rank: number) => {
    ...
    try {
      ...
      // ✅ CORREÇÃO: Validação do dia é feita no backend (api/claimPrize)
      // O backend verifica se claimableDay >= todayDay e rejeita se for dia atual ou futuro
      // Não precisamos verificar totalPlayers aqui - se chegou aqui e é dia passado, está ok
  ...
  // Isso garante que os rankings e claims sejam recarregados quando o dia muda
  useEffect(() => {
      if (displayDate) {
        // ✅ CORREÇÃO: Sempre recarregar ranking quando displayDate mudar
                      ...
                      <tr key={`${row.player}-${rank}`}>
                        ...
                        <td className="px-4 py-3 text-center">
                          ...
                          {canClaimResult ? (
                            ...
                            // ✅ CORREÇÃO: Verificar se é dia passado usando cálculo UTC local
                            const selectedDayFromDisplay = displayDate ? getDayId(new Date(displayDate + 'T00:00:00Z')) : null
                            const todayDayFromDisplay = getDayId()
                            const isPastDayCheck = selectedDayFromDisplay !== null && selectedDayFromDisplay < todayDayFromDisplay
                            const isCurrentPlayer = row.player?.toLowerCase() === currentPlayer?.toLowerCase()
                            
                            // Se não é dia passado e é o jogador atual no top 3, mostrar mensagem
                            if (!isPastDayCheck && rank <= 3 && isCurrentPlayer) {
